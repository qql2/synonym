...
export class SynonymCore {
	...
	static addCommand(plugin: Synonym) {
		plugin.addCommand({
			...
			editorCallback: async () => {
				plugin.settings.ignoreCase = plugin.settings.ignoreCase
					? false
					: true;
	...
	collectMatchedSynonym(matchMap: Map<string, MATCH_RST>): string[] {
		...
		for (const matchRst of matchMap.values()) {
			if (matchRst.specialMeaning.length)
				synonym.push(...matchRst.specialMeaning);
			else if (matchRst.generalized.length)
				synonym.push(...matchRst.generalized);
		}
		...
		for (const synonym of synonyms) {
			...
			for (const synonymSnippet of synonymSnippets) {
				if (SynonymCore.isSynonym(synonymSnippet))
					expendKeys.push(
						...SynonymCore.splitSynonymSet(synonymSnippet)
					);
			}
		}
		return expendKeys;
	...
	matchSynonymBySceneKeys(
		...
		for (const key of keys) {
			...
			let matches = this.matchKeyWithAnnotation(
				targetSynonymLst,
				key,
				ignoreCase
			);
			for (const match of matches) {
				if (!match.groups.annotation) {
					...
					this.isSpecial(match.groups.annotation, restK, ignoreCase)
				)
					rst.specialMeaning.push(key);
				// if (key == 'bb') debugger
			}
		}
	 ...
	 */
	static matchSynonymByIndependentKeys(
		targetSynonym: string,
		keys: string[],
		ignoreCase = true
	) {
	...
	private isSpecial(
		...
		let synonymSets = Split.splitWithBracket(
			annotation,
			splitRgx,
			0
		).filter((v) => v);
		...
		for (const sySet of synonymSets) {
			let matchRst = this.matchSynonymBySceneKeys(
				sySet,
				SceneKeys,
				ignoreCase
			);
	...
	/** 将注释内容转换为逻辑表达式进行求值 */
	private calcAnnotation(
		annotation: string,
		synonymBoolean: Map<string, boolean>
	): boolean {
		let express = annotation;
		/* 把同义词替换为布尔值 */
		for (const sy of synonymBoolean) {
			// 使用字符串替换而不是正则表达式，避免重复编译
			express = this.replaceAllOccurrences(express, sy[0], `${sy[1]}`);
		}
		/* 把自定义括号替换为圆括号 */
	...
	private replaceAllOccurrences(
		...
		while (true) {
			const index = result.indexOf(searchValue, startIndex);
			if (index === -1) break;

			result =
				result.slice(0, index) +
				replaceValue +
				result.slice(index + searchValue.length);
			startIndex = index + replaceValue.length;
		}
	 ...
	 */
	private matchKeyWithAnnotation(
		targetSynonym: string,
		targetK: string,
		ignoreCase = true
	) {
		...
		for (const key of SynonymCore.splitSynonymSet(targetSynonym)) {
			const match = this.matchKeyWithAnnotationNoRegex(
				key,
				targetK,
				ignoreCase
			);
			if (match) {
				// 创建兼容RegExpMatchArray的对象，保持现有接口
				const fakeMatch = [key] as unknown as RegExpMatchArray;
				fakeMatch.groups = { annotation: match.annotation };
				fakeMatch.index = 0;
				fakeMatch.input = key;
				rst.push(fakeMatch);
			}
	...
	/**
	 * 不使用正则表达式的字符串匹配方法
	 * 替代原正则: (（(?<annotation>.*?)）)?(?<=）|^)${targetK}$
	 */
	private matchKeyWithAnnotationNoRegex(
		text: string,
		targetK: string,
		ignoreCase = true
		...
		const searchTarget = ignoreCase ? targetK.toLowerCase() : targetK;

		// 1. 检查是否以targetK结尾
		if (!searchText.endsWith(searchTarget)) {
			return null;
		}

		// 2. 获取targetK前面的部分
		const beforeTarget = text.slice(0, text.length - targetK.length);

		// 3. 情况1: 直接就是targetK (对应正则中的 ^targetK$)
		if (beforeTarget === "") {
			return { annotation: undefined };
		}

		// 4. 情况2: 检查是否有注释格式 （...）targetK
		if (beforeTarget.endsWith("）")) {
			const annotation = this.extractAnnotation(beforeTarget);
			if (annotation !== null) {
				return { annotation };
			}
		}
	...
	private extractAnnotation(text: string): string | null {
		...
		const openPos = text.lastIndexOf("（");
		...
		// 检查（是否在字符串开始位置（对应正则中的(?<=^)）
		if (openPos !== 0) {
			return null;
		}

		// 提取并返回注释内容
		return text.slice(openPos + 1, closePos);
	}
	/** @return 返回若干个同义词，已去空处理 */
	static splitSynonymSet(synonym: string): string[] {
		if (!SynonymCore.isSynonym(synonym))
			throw new Error(`接受的参数:"${synonym}"不是一个同义词标签`);
		return Split.splitWithBracket(synonym, /-/g, 0, 0, [
			{ left: "（", right: "）" },
		]).filter((v) => v);
	}
}
