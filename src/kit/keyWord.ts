...
export class KeyWord {
    /** @return 已去重处理 */
    static collectKeyWordsAndExtend(data: xunfeiData[], context: string) {
        let rst: string[]
        if (data.length == 0) {
            return []
        }
    ...
    static joinKeys(keys: string[], context: string) {
        ...
        context = this.markKeys(context, keys, delimiters)
     ...
     * 使用标记符包围上下文中包含的所有关键词
     */
    static markKeys(text: string, keys: string[], delimiters: string[]): string {
        let rst
        let regex = new RegExp(`${keys.join('|')}}`, 'g')
        rst = text.replace(regex, `${delimiters[0]}$&${delimiters[1]}`)
        return rst
    }
    static countKeys(context: string, keysRegex: RegExp): number {
        return Array.from(context.matchAll(keysRegex)).length
    }
